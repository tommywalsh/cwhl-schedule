...
# - We'll have two fully-nondivisional weeks, each with 5 non-divisional games
# - We'll also have 15 "hybrid" mostly-divisional weeks, each with 4 divisional games, and 1 non-divisional game
#   - The hybrid weeks are broken up into three groups of five weeks.
#   - Each group's games are generated by the modified round-robin algorithm described below
#
# Modified round-robin algorithm for the mostly-divisional weeks:
...
wales = ["Steamers", "Army", "Cold Necks", "Pleased", "Magoons"]
campbell = ["Pants", "Breakfast", "Loonies", "Zaus", "Poachers"]
...
# Uses round-robin algorithm to generate a full slate of games for a hybrid week
...
def generate_games_for_hybrid_week(week_num, division1, division2, division_offset, parity):
    ...
    # A hybrid week has one non-divisional game
...
# Generates a full slate of games for a non-divisional week
def generate_games_for_nondiv_week(week, division1, division2):
    assert (len(division1) == len(division2))
    teams_per_division = len(division1)
    parity = (week % 2) == 1
    ...
    return {
        "title": f"Non-Divisional Week #{week + 1}",
        "games": games
    }
...
def generate_all_weeks(divison1, division2):
    ...
    # We want two non-divisional weeks (with the divisions being offset by 0 then 1)...
    all_weeks = [
        generate_games_for_nondiv_week(0, divison1, division2),
        generate_games_for_nondiv_week(1, divison1, division2)
    ]

    # ...and three sets of five hybrid weeks, with the divisions offset by 2 then 3 then 4 in each set.
    for set_number in range(0, 3):
        ...
        for week in range(0, 5):
            all_weeks.append({
                "title": f"Hybrid Week #{set_number*5 + week + 1}",
                "games": generate_games_for_hybrid_week(week, divison1, division2, 2 + set_number, parity)
            })
